...
class MemberPromoteTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('member_promote', kwargs={'member_id': self.member.id})

    def test_member_promote_url(self):
        self.assertEqual(self.url,f'/member_promote/{self.member.id}')

    def test_member_promote_when_not_logged_in(self):
        response = self.client.post(self.url, follow=True)
        redirect_url = reverse("log_in")
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)

    def test_member_promote_when_logged_in_but_invalid_member_id(self):
        self.client.login(email=self.user.email, password='Password123')
        url = reverse('member_promote', kwargs={'member_id': self.member.id+9999})
        response = self.client.get(url)
        redirect_url = reverse("club_list")
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)

    def test_member_promote_when_logged_in_with_valid_member_id(self):
        self.client.login(email=self.user.email, password='Password123')
